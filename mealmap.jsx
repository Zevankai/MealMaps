...
// ─── Storage ───
const SKEY = "mealmap-data-v2";
const load = async () => { try { const r = await window.storage.get(SKEY); return r ? JSON.parse(r.value) : null; } catch { return null; } };
const save = async (d) => { try { await window.storage.set(SKEY, JSON.stringify(d)); } catch (e) { console.error(e); } };
...
export default function MealMap() {
  ...
  useEffect(() => { load().then((d) => { if (d) setData({ ...defaults, ...d }); setLoaded(true); }); }, []);
